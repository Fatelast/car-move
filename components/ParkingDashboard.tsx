...
const ParkingDashboard: React.FC<ParkingDashboardProps> = ({ config, onStop }) => {
  ...
  const {
    ...
    return {
      totalDurationFormatted: `${hours}小时 ${minutes}分钟`,
  ...
  useEffect(() => {
    ...
    const remainingMinutesValue = Math.ceil(remainingMs / 60000);
    try {
      new Notification('挪车宝提醒', {
        body: `快去挪车！距离下一个计费周期仅剩 ${remainingMinutesValue} 分钟。`,
        icon: '/vite.svg',
    ...
    audio.play().catch((error) => console.log('Audio play failed interaction required', error));
    setLastNotificationKey(currentKey);
  }, [cycleCount, lastNotificationKey, permission, remainingMs, status]);
    ...
    <div className="flex flex-col items-center justify-between min-h-[90vh] py-6 px-4 w-full max-w-md mx-auto animate-fade-in relative">
      ...
      <div className="relative w-72 h-72 flex items-center justify-center mb-8">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center z-10 pointer-events-none">
          ...
          <div className={`text-5xl font-mono font-bold tracking-tighter transition-all duration-300 ${
            status === ParkingStatus.DANGER
              ? 'text-[#9b2d3b] animate-pulse-fast scale-110'
              : status === ParkingStatus.WARNING
                ? 'text-[#e67e5b] animate-pulse'
                : 'text-slate-800'
          }`}
          >
            {remainingMinutes}
          ...
          {status !== ParkingStatus.SAFE && (
            <div className={`mt-3 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold shadow-sm transition-all duration-300 ${
              status === ParkingStatus.DANGER
                ? 'bg-[#fff5f5] text-[#9b2d3b] border border-[#9b2d3b]/20 scale-105'
                : 'bg-[#fffbf0] text-[#f09e5c] border border-[#f09e5c]/20'
            }`}
            >
      ...
      {showImageModal && config.locationImage && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4 animate-fade-in" onClick={() => setShowImageModal(false)}>
          <div className="relative w-full max-w-lg bg-white p-2 rounded-2xl shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <button
      ...
      {showStopConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-brand-900/20 backdrop-blur-sm p-6 animate-fade-in" onClick={() => setShowStopConfirm(false)}>
          <div className="bg-white rounded-3xl border border-brand-100 w-full max-w-sm p-6 shadow-2xl" onClick={(event) => event.stopPropagation()}>
