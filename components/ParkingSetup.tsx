...
const ParkingSetup: React.FC<ParkingSetupProps> = ({ onStart, onViewHistory }) => {
  ...
  const [cycleCost, setCycleCost] = useState(5);
  ...
  const [startTimeStr, setStartTimeStr] = useState(getNowString());
  ...
  const adjustCost = (delta: number) => {
    setCycleCost((previous) => Math.max(0, Math.min(200, previous + delta)));
  };

  const adjustReminder = (delta: number) => {
    setReminder((previous) => Math.max(1, Math.min(59, previous + delta)));
  };
    ...
    <div className="flex flex-col items-center justify-center min-h-[85vh] px-4 w-full max-w-md mx-auto animate-slide-up pb-10">
      ...
      <div className="text-center mb-6">
        <div className="relative inline-block">
          <div className="absolute inset-0 bg-brand-300 rounded-full blur-xl opacity-40 animate-pulse" />
      ...
      <div className="w-full bg-white rounded-3xl p-6 border border-brand-100/50 shadow-[0_20px_40px_-15px_rgba(230,126,91,0.1)] space-y-7">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center bg-brand-50 rounded-lg border border-brand-100 overflow-hidden">
              ...
              <div className="w-10 text-center font-bold text-brand-800 text-sm">
                {cycleCost}
                <span className="text-[10px] ml-0.5">元</span>
              </div>
              <button
                onClick={() => adjustCost(1)}
                className="w-8 h-8 flex items-center justify-center text-brand-600 active:bg-brand-100"
            ...
            {PRESETS.map((preset) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-[10px] mt-0.5 ${isActive ? 'text-brand-600/80' : 'text-slate-400'}`}>
                    {preset.desc}
                  </span>
                </button>
              );
          ...
          <div className="relative py-2">
            ...
            <div className="relative flex justify-center">
              <span className="bg-white px-2 text-xs text-slate-400">或自定义</span>
            </div>
          </div>

          <div className="grid grid-cols-3 gap-3">
            {[30, 60, 15].map((minutes) => (
              <button
                key={minutes}
              ...
              >
                {minutes === 60 ? '1小时/周期' : `${minutes}分钟/周期`}
              </button>
            ))}
          </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <div className={`flex-[1.5] flex items-center rounded-xl border transition-all duration-200 bg-brand-50 ${
              ![5, 10, 15].includes(reminder) ? 'border-brand-500 ring-1 ring-brand-500/20' : 'border-brand-100'
            }`}
            >
              ...
              <div className="flex-1 flex items-center justify-center relative">
                ...
                />
                <span className="absolute right-0 text-xs text-slate-400 pointer-events-none">分钟</span>
              </div>
        ...
        <div className="space-y-3">
          <label className="text-sm font-semibold text-slate-700 flex justify-between items-center">
            ...
            {locationImage && (
              <button
                ...
                className="px-2 py-1 bg-slate-100 rounded text-xs text-slate-500 font-medium active:scale-95"
              >
                删除照片
              </button>
            )}
          ...
          {!locationImage ? (
            ...
            <div
              className="relative w-full h-36 rounded-2xl overflow-hidden border border-brand-200 shadow-md group cursor-pointer active:scale-[0.98] transition-transform"
              onClick={() => fileInputRef.current?.click()}
            >
